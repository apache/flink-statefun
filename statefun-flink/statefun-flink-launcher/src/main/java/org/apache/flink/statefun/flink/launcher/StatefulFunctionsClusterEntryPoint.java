  ...
  private static void addStatefulFunctionsConfiguration(Configuration configuration) {
    ...
    if (parentFirst.isEmpty()) {
      configuration.set(
          ...
          Collections.singletonList(Constants.STATEFUL_FUNCTIONS_PACKAGE));
    } else {
      parentFirst.add(Constants.STATEFUL_FUNCTIONS_PACKAGE);
      configuration.set(CoreOptions.ALWAYS_PARENT_FIRST_LOADER_PATTERNS_ADDITIONAL, parentFirst);
    }
  }
