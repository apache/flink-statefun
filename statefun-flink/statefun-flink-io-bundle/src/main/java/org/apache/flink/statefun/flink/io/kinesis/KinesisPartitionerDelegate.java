...
final class KinesisPartitionerDelegate<T> extends KinesisPartitioner<T> {

  private static final long serialVersionUID = 1L;

  private final KinesisEgressSerializer<T> delegate;

  KinesisPartitionerDelegate(KinesisEgressSerializer<T> delegate) {
    this.delegate = Objects.requireNonNull(delegate);
  }

  @Override
  public String getPartitionId(T element) {
    return delegate.partitionKey(element);
  }

  @Override
  public String getExplicitHashKey(T element) {
    return delegate.explicitHashKey(element);
  }
}
