...
public final class AutoRoutableKafkaIngressDeserializer
  ...
  public AutoRoutableKafkaIngressDeserializer(Map<String, RoutingConfig> routingConfigs) {
  ...
  private byte[] requireNonNullKey(byte[] key) {
    if (key == null) {
      TypeName tpe = AutoRoutableKafkaIngressComponentBinderV1.KIND_TYPE;
      throw new IllegalStateException(
          "The "
              + tpe.namespace()
              + "/"
