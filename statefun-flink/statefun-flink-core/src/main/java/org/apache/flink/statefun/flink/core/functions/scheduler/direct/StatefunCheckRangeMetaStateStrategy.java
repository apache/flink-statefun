...
public class StatefunCheckRangeMetaStateStrategy extends SchedulingStrategy {
    ...
    public StatefunCheckRangeMetaStateStrategy(){ }
    ...
    public void initialize(LocalFunctionGroup ownerFunctionGroup, ApplyingContext context){
        ...
        if(!RANDOM_LESSEE){
            this.lesseeSelector = new RRIdSpanLesseeSelector(((ReusableContext) context).getPartition(), SEARCH_RANGE, ID_SPAN);
        }
        else{
        ...
        this.flushRequests = new HashMap<>();
        this.messageCount = 0;
        LOG.info("Initialize StatefunCheckRangeDirectStrategy with SEARCH_RANGE {} ID_SPAN {} RANDOM_LESSEE {}", SEARCH_RANGE, ID_SPAN, RANDOM_LESSEE);
    }
...
//                            + " workqueue size " + workQueue.size() + " pending message queue size " +  idToMessageBuffered.size());
                ...
                HashSet<Message> completeFlushRequests = new HashSet<>();
                flushRequests.forEach((key, counter) -> {
                    if (idToMessageBuffered.keySet().stream().noneMatch(k -> k <= counter)) {
                        completeFlushRequests.add(key);
                    }
                });
                if(!completeFlushRequests.isEmpty()){
                    for(Message complete : completeFlushRequests){
                        context.send(complete);
                        flushRequests.remove(complete);
                    }
                }
    ...
    public Message prepareSend(Message message){
        if(context.getMetaState() != null ) {
            ...
            if(!state.redirectable && !state.flushing){
                return message;
            }
            else if (!state.redirectable && state.flushing){
                flushRequests.put(message, messageCount);
                return null;
            }
        }
