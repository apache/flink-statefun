                        ...
                        .map(
                                name ->
                                        RoutableMessageBuilder.builder()
                                                .withTargetAddress(GREET, "target")//name)
...
//    private final PersistedValue<Integer> seenCount = PersistedValue.of("seen", Integer.class);
        private final HashMap<FunctionType, StatefulFunction> registeredFunctions = new HashMap<>();
        @Persisted
        ...
        public void invoke(Context context, Object input) {
            if (input instanceof FunctionRegistration){
                ...
                if (!registeredFunctions.containsKey(registration.functionType)){
                    registeredFunctions.put(registration.functionType, registration.statefulFunction);
                }
            }
            else if (input instanceof FunctionInvocation){
                FunctionInvocation localInvocation = (FunctionInvocation)input;
                StatefulFunction function = registeredFunctions.getOrDefault(localInvocation.functionType, null);
                if (function == null){
                    System.out.println("Functiontype " + localInvocation.functionType + " is never registered at " + this.toString());
                }
                else{
...
//    }
    ...
    private static final class NameSource implements SourceFunction<Object> {
        ...
        @Override
        public void run(SourceContext<Object> ctx) throws InterruptedException {
//            String[] names = {"Stephan", "Igal", "Gordon", "Seth", "Marta"};
            ...
            synchronized (ctx.getCheckpointLock()) {
                ...
                ctx.collect(new FunctionRegistration(GREET3, new MyFunction3()));
            }
            ...
            while (true) {
                ...
                synchronized (ctx.getCheckpointLock()) {
                    ...
                    ctx.collect(new FunctionInvocation(GREET3, "random text "+ count));
                }
                //Thread.sleep(10);
                if (count++ > 10) {
                    break;
